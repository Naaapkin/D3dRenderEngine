...
#include "Engine/Render/PC/Private/D3D12SwapChain.h"
...
void D3D12RHI::Initialize()
{
    ...
    static RootSignatureLayout GLOBAL_ROOT_SIG_LAYOUT = { 4, 3, 2 };
    ...
    mRootSignatureManager->AppendSignature("UniversalRootSignature", rootSignatureDesc, GLOBAL_ROOT_SIG_LAYOUT);
    // ------------------------------------------------------------------------
...
{
    RHINativeBuffer* pBuffer = new D3D12UploadBuffer(mStagingBufferAllocator->GetD3D12Resource(), mStagingBufferAllocator->Allocate(size));
    return std::make_unique<RHIStagingBuffer>(std::unique_ptr<RHINativeBuffer>(pBuffer));
}

std::unique_ptr<RHIStagingBuffer> D3D12RHI::RHIAllocStagingBuffer(const RHITextureDesc& desc, uint8_t mipmap)
{
    ResourceDimension dimension = desc.mDimension;
    ...
    RHINativeBuffer* pBuffer = new D3D12UploadBuffer(mStagingBufferAllocator->GetD3D12Resource(), mStagingBufferAllocator->AllocateAligned(size, D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT));
    return std::make_unique<RHIStagingBuffer>(std::unique_ptr<RHINativeBuffer>(pBuffer));
}

std::unique_ptr<RHIConstantBuffer> D3D12RHI::RHIAllocConstantBuffer(uint64_t size)
 {
     RHINativeBuffer* pBuffer = new D3D12UploadBuffer(mBuddyCBufferAllocator->GetD3D12Resource(),
         mBuddyCBufferAllocator->Allocate(
	         ::AlignUpToMul<uint64_t, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT>()(size)));
     return std::make_unique<RHIConstantBuffer>(std::unique_ptr<RHINativeBuffer>(pBuffer));
 }

std::unique_ptr<RHIInstanceBuffer> D3D12RHI::RHIAllocInstanceBuffer(uint64_t size)
{
    RHINativeBuffer* pCBuffer = new D3D12UploadBuffer(mBuddyCBufferAllocator->GetD3D12Resource(),
        mBuddyCBufferAllocator->Allocate(
            ::AlignUpToMul<uint64_t, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT>()(size)));
    return std::make_unique<RHIInstanceBuffer>(std::unique_ptr<RHINativeBuffer>(pCBuffer));
}
...
{
    ASSERT(desc.mDimension == ResourceDimension::TEXTURE2D, TEXT("depth stencil buffer should be 2-dimension texture."));
    desc.mMipLevels = desc.mMipLevels ? desc.mMipLevels : GetMipLevelCount(desc.mWidth, desc.mHeight, desc.mDepth);
    D3D12_HEAP_PROPERTIES prop = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    ...
    D3D12FrameBuffer* pTexture = new D3D12FrameBuffer(desc, pResource.Detach(), dsvHandle);
    ResourceStateTracker::AppendResource(pTexture->GetD3D12Resource(), ResourceState::COMMON);
    return std::make_unique<RHIDepthStencil>(std::unique_ptr<D3D12FrameBuffer>(pTexture));
}
...
std::unique_ptr<RHIRenderTarget> D3D12RHI::RHIAllocRenderTarget(RHITextureDesc desc)
{
    ...
    D3D12FrameBuffer* pTexture = new D3D12FrameBuffer(desc, pResource.Detach(), rtvHandle);
    ResourceStateTracker::AppendResource(pTexture->GetD3D12Resource(), ResourceState::PRESENT);
    return std::make_unique<RHIRenderTarget>(std::unique_ptr<D3D12FrameBuffer>(pTexture));
}

std::unique_ptr<RHIVertexBuffer> D3D12RHI::RHIAllocVertexBuffer(uint32_t vertexSize,
                                                                uint32_t numVertices)
{
    ...
    UComPtr<ID3D12Resource> pResource = mDevice->CreateCommitedResource(prop, D3D12_HEAP_FLAG_NONE, d3d12Desc, nullptr,
        D3D12_RESOURCE_STATE_COMMON);
    D3D12VertexBuffer* pVertexBuffer = new D3D12VertexBuffer(std::move(pResource), RHIBufferDesc{ size, vertexSize, ResourceType::STATIC }, vertexSize, numVertices);
...
std::unique_ptr<RHIIndexBuffer> D3D12RHI::RHIAllocIndexBuffer(uint64_t numIndices, Format indexFormat)
{
    ...
    D3D12_HEAP_PROPERTIES prop = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    D3D12_RESOURCE_DESC d3d12Desc = CD3DX12_RESOURCE_DESC::Buffer(stride * numIndices);
    UComPtr<ID3D12Resource> pResource = mDevice->CreateCommitedResource(prop, D3D12_HEAP_FLAG_NONE, d3d12Desc, nullptr,
        D3D12_RESOURCE_STATE_COMMON);
    D3D12IndexBuffer* pIndexBuffer = new D3D12IndexBuffer(std::move(pResource), RHIBufferDesc{ stride * numIndices, stride, ResourceType::STATIC }, numIndices, indexFormat);
...
std::unique_ptr<RHISwapChain> D3D12RHI::RHICreateSwapChain(const RHISwapChainDesc& desc)
{
    ...
    std::unique_ptr<D3D12FrameBuffer*[]> renderTargets;
    renderTargets.reset(new D3D12FrameBuffer *[desc.mNumBackBuffers]);
    RHITextureDesc backBufferDesc = {desc.mFormat, ResourceDimension::TEXTURE2D, 1, 1, 1, 1, desc.mMSAA, static_cast<uint8_t>(desc.mMSAA > 1 ? 1 : 0)};
    for (int i = 0; i < desc.mNumBackBuffers; ++i)
    {
        ...
        D3D12FrameBuffer* pRenderTarget = new D3D12FrameBuffer(backBufferDesc, pD3D12BackBuffer, rtvHandle);
        ResourceStateTracker::AppendResource(pRenderTarget->GetD3D12Resource(), ResourceState::PRESENT);
        renderTargets[i] = pRenderTarget;
    }

    return std::make_unique<D3D12SwapChain>(std::move(pSwapChain), renderTargets.get(), desc.mNumBackBuffers);
}

void D3D12RHI::RHIUpdateStagingBuffer(RHIStagingBuffer* pBuffer, const void* pData, uint64_t offset,
                                      uint64_t size)
{
    static_cast<D3D12UploadBuffer*>(pBuffer->GetBuffer())->Update(pData, offset, size);
}

void D3D12RHI::RHIUpdateStagingBuffer(RHIStagingBuffer* pBuffer, const RHITextureDesc& desc, const void* pData, uint8_t mipmap)
{
    D3D12UploadBuffer* pNativeBuffer = static_cast<D3D12UploadBuffer*>(pBuffer->GetBuffer());
	...
	case ResourceDimension::TEXTURE2D:
        ...
        else
        {
            ...
            {
                pNativeBuffer->Update(pSrc, 0, rowSize);
            }
        }
        break;
	case ResourceDimension::TEXTURE3D:
...
void D3D12RHI::RHIUpdateInstanceBuffer(RHIInstanceBuffer* pBuffer, const void* pData, uint64_t offset, uint64_t size)
{
    static_cast<D3D12UploadBuffer*>(pBuffer->GetBuffer())->Update(pData, offset, size);
}

void D3D12RHI::RHIReleaseConstantBuffer(RHIConstantBuffer* pBuffer)
{
    D3D12UploadBuffer* pUploadBuffer = static_cast<D3D12UploadBuffer*>(pBuffer->GetBuffer());
    mBuddyCBufferAllocator->Free(pUploadBuffer->Offset());
}

void D3D12RHI::RHIReleaseInstanceBuffer(RHIInstanceBuffer* pBuffer)
{
    D3D12UploadBuffer* pUploadBuffer = static_cast<D3D12UploadBuffer*>(pBuffer->GetBuffer());
    mBuddyCBufferAllocator->Free(pUploadBuffer->Offset());
}
...
void D3D12RHI::Release()
{
    ...
    mCommandObjectPool.reset();
    mPipelineStateManager.reset();
    mRootSignatureManager.reset();
    mRTVAllocator.reset();
    mDSVAllocator.reset();
    mOnlineCBVSRVUAVAllocator.reset();
    mRingCBufferAllocator.reset();
    mBuddyCBufferAllocator.reset();
    mStagingBufferAllocator.reset();
    mDevice.reset();
}
...
void D3D12RHI::GetShaderProperties(ShaderType type, ID3D12ShaderReflection* pReflector, std::unordered_set<ShaderProp>& properties)
{
    ...
    for (uint32_t i = 0; i < shaderDesc.BoundResources; ++i)
    {
        ...
        case D3D_SIT_TBUFFER:
            property.mType = ShaderPropType::TBUFFER;
            break;
        case D3D_SIT_TEXTURE:
            property.mType = ShaderPropType::TEXTURE;
            property.mInfo.mDimension = ::ConvertFromD3DSRVFormat(bindingDesc.Dimension);
            break;
        case D3D_SIT_SAMPLER:
            property.mType = ShaderPropType::SAMPLER;
            // TODO:
            if (property.mRegister < 4) continue;   // skip static samplers , TODO:
            break;
        default:
            break;
        }
	                                        ...
	                                        D3D12_STATIC_SAMPLER_DESC>& samplers)
{
    ...
    params[2].InitAsConstantBufferView(2, 0, D3D12_ROOT_DESCRIPTOR_FLAG_NONE);
    // | shadow map, depth buffer, opaque buffer.... material textures | material constants | 
    ranges[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, layout.mNumMaterialConstants, 3);
    ranges[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, layout.mNumTextures, 0);
    params[3].InitAsDescriptorTable(2, ranges.get());
