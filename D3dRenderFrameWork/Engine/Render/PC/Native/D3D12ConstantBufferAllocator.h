...
struct Allocation
{
	Allocation() : mGPUAddress(0), mCPUAddress(nullptr), mOffset(MAXUINT64), mSize(0) { }
...
inline const D3D12Resource* D3D12BuddyBufferAllocator::GetD3D12Resource() const
{
...
inline void D3D12BuddyBufferAllocator::Initialize(D3D12Device* pDevice, uint64_t poolSize)
{
    ...
    mResource = std::make_unique<D3D12Resource>(
	    mDevice->CreateCommitedResource(heapProps, D3D12_HEAP_FLAG_NONE, desc, nullptr,
