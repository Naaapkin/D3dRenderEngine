...
{
public:
    RHINativeTexture* GetBuffer() const { return mTexture.get(); }
...
#define DERIVE_TEXTURE_WRAPPER(className) \
...
DERIVE_BUFFER_WRAPPER(RHIInstanceBuffer);
DERIVE_BUFFER_WRAPPER(RHIStagingBuffer);
DERIVE_BUFFER_WRAPPER(RHIVertexBuffer);
DERIVE_BUFFER_WRAPPER(RHIIndexBuffer);

DERIVE_TEXTURE_WRAPPER(RHIRenderTarget);
DERIVE_TEXTURE_WRAPPER(RHIDepthStencil);
//
...
{
public:
    virtual void UpdateBuffer(RHIConstantBuffer* pCBuffer, void* pData, uint64_t offset, uint64_t size) = 0;
...
public:
    ...
    virtual std::unique_ptr<RHIConstantBuffer> AllocTempConstantBuffer(uint16_t size) = 0;
    virtual std::unique_ptr<RHIInstanceBuffer> AllocTempInstanceBuffer(uint32_t size, uint32_t stride) = 0;
    virtual void UpdateConstantBuffer(RHIConstantBuffer* pCBuffer, void* pData, uint64_t offset, uint64_t size) = 0;
    virtual void UpdateInstanceBuffer(RHIInstanceBuffer* pBuffer, void* pData, uint64_t offset, uint64_t size) = 0;
    virtual void UpdateBuffer(RHIBufferWrapper* pDst, RHIStagingBuffer* pStagingBuffer, uint64_t size, uint64_t dstStart, uint64_t srcStart) = 0;
    ...
    virtual void SetInstanceBuffer(uint8_t slot, RHIInstanceBuffer* pInstanceBuffer) = 0;
    ...
    virtual void SetRenderTargetsAndDepthStencil(RHIRenderTarget* const* renderTargets, uint32_t numRenderTargets, RHIDepthStencil const* depthStencilTarget) = 0;
