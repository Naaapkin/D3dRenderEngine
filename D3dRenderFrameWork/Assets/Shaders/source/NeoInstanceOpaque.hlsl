#include "common/Samplers.hlsl"
#include "common/Common.hlsl"

BEGIN_VIEW_DATA
END_VIEW_DATA

BEGIN_INSTANCE_DATA
END_INSTANCE_DATA

BEGIN_MATERIAL_DATA(DiffuseInfo, b3)
    float4 m_diffuseBias;
END_MATERIAL_DATA

struct InstanceVertexInput
{
    float3 positionOS : POSITION;
    float3 normalOS : NORMAL;
    float2 uv : TEXCOORD;
    uint   instanceId : SV_InstanceID;
};

struct FragInput
{
    float4 positionHS : SV_POSITION;
    float3 normalWS : NORMAL;
    float2 uv : TEXCOORD0;
};

Texture2D<float4> diffuse : register(t1);

FragInput VsMain(InstanceVertexInput input)
{
    FragInput o;
    float4 worldPosition = mul(InstanceBuffer[input.instanceId].m_model, float4(input.positionOS, 1));
    o.positionHS = mul(m_projection, mul(m_view, worldPosition));
    o.normalWS = mul((float3x3) transpose(InstanceBuffer[input.instanceId].m_model_i), input.normalOS);

    o.uv = input.uv * m_diffuseBias.xy + m_diffuseBias.zw;
    return o;
}

float4 PsMain(FragInput input) : SV_TARGET
{
    return float4(input.normalWS, 1);
    //diffuse.Sample(LinearSampler, input.uv);
}

